...
function clusterClients3(clustersdico,traveltimes,traveltimes2)
    selected1 = 0
    selected2 = 0
    best = 0
    maxcapcity = 1*4*60
    nclusters = maximum(values(clustersdico))
    clusters = [ findall(==(1),clustersdico) ]
    for i=1:nclusters-1
        for j=i+1:nclusters
            if i == 1
                push!(clusters,findall(==(j),clustersdico))
            end
            timesbetween = maximum(traveltimes[clusters[i],clusters[j]]) + maximum(getTravelTime(traveltimes,0,clusters[i])) + maximum(getTravelTime(traveltimes,0,clusters[j]))
            timeswithin = maximum(traveltimes[clusters[i],clusters[i]]) + maximum(traveltimes[clusters[j],clusters[j]])
            #=
    ...
    if best == 0
        ...
        #
        for index in eachindex(clustersdico)
            if clustersdico[index] == selected2
                clustersdico[index] = selected1
            elseif clustersdico[index] > selected2
                clustersdico[index] -= 1
            end
        end
        #
        clusterClients3(clustersdico,traveltimes,traveltimes2)
    end
end
